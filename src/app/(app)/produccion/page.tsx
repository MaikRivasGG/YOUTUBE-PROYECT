...
import { getActivity, getBoardVideos, getUpcoming, getWorkspaceStats } from "@/server/queries";
...
export default async function ProductionPage({
  ...
  const [videos, stats, upcoming, activity] = await Promise.all([
  ...
  const weekPublications = videos.filter((video) => {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <PageHeader title="Pipeline de producción" />
      ...
      <div className="scrollbar-slim flex min-h-0 flex-1 gap-5 overflow-y-auto px-5 py-4 lg:px-7 xl:overflow-hidden">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-4 xl:overflow-hidden">
          ...
          <Board initialVideos={videos} initialChannelFilter={canal ?? null} />
        </div>
